---
name: tiger-release
description: Prepare and publish a TIGER release by keeping implementation, tests, documentation, citations, version metadata, GitHub, and PyPI artifacts synchronized. Use when adding a releasable TIGER feature or fix, preparing a version, merging its release PR, or publishing graph-tiger.
---

# TIGER Release

Use this skill for end-to-end changes that must reach both the TIGER repository and the `graph-tiger` package. A release is complete only when the merged source, documentation, version metadata, GitHub release, and PyPI artifacts agree.
