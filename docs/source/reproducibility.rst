...
.. raw:: html
   ...
   <p>Download <a href="guide-results/run-guide-studies.py">run-guide-studies.py</a>, <a href="guide-results/run-extended-studies.py">run-extended-studies.py</a>, <a href="guide-results/run-influence-guide.py">run-influence-guide.py</a>, and <a href="guide-results/run_guide_bootstrap.py">run_guide_bootstrap.py</a> into the same directory. Then create an environment and install the <a href="guide-results/requirements-lock.txt">recorded dependency versions</a>. The recorded environment used Python 3.12.</p><pre><code>python -m venv .venv
   .venv\Scripts\python -m pip install -r requirements-lock.txt
   .venv\Scripts\python run-guide-studies.py defenses
    .venv\Scripts\python run-extended-studies.py
    .venv\Scripts\python run-influence-guide.py</code></pre><p>On macOS/Linux use <code>.venv/bin/python</code>. Omitting <code>defenses</code> runs the five core guide studies; other choices are <code>attacks</code>, <code>epidemics</code>, <code>cascades</code>, and <code>visualization</code>. The influence runner reproduces its four model figures and shared result files. From a downloaded copy, it writes to <code>site/guide-results</code> relative to the runner; the copy already stored in <code>guide-results</code> writes beside itself. The approximation study has its own <a href="approximation-results/run-approximation.py">runner</a> and <a href="approximation-results/render-approximation.py">renderer</a>; keep both together.</p>
